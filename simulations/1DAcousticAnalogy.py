...
# Precompute Gaussian envelope for the source
grid_indices = np.arange(nx)
gaussian_envelope = np.exp(-((grid_indices - source_position) ** 2) / (2 * source_width ** 2))
...
for n in range(time_steps):
    ...
    Ez += np.sin(2 * np.pi * source_frequency * n * dt) * gaussian_envelope
...
# Ez_line, = ax.plot([], [], label=r'$T_{21}$')
# Hy_line, = ax.plot([], [], label=r'$v_y$')
...
def plot_frame(frame=-1, save=False, filename="", show=True):
    # Animation setup
    fig = plt.figure(figsize=(10, 5))
    ax = fig.add_subplot(111)

    Ez_line, = ax.plot(range(nx), Ez_history[frame], label=r'$T_{21}$')
    Hy_line, = ax.plot(range(nx), Hy_history[frame], label=r'$v_y$')
...
def save_frames(skip_frames=1, foldername="Frames", frame_name="frame"):
    if not os.path.exists(FIGS+foldername):
        os.makedirs(FIGS+foldername)
    for i, frame in enumerate(range(0, time_steps, skip_frames)):
        file_name = frame_name+str(i+1)
        plot_frame(frame, save=True, filename=foldername+"/"+file_name, show=False)


save_frames(skip_frames=5, foldername="AcousticTv", frame_name="AcousticTv")
