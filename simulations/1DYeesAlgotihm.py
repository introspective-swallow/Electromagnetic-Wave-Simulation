...
dt = dx / (2 * c)  # Time step satisfying the CFL condition

# Source parameters
source_frequency = 1e9  # Source frequency (Hz)
source_position = nx // 2  # Center of the spatial grid
source_width = 20  # Width of the Gaussian envelope

# Fields and currents
Ez = np.zeros(nx)  # Electric field (z-direction)
Hy = np.zeros(nx)  # Magnetic field (y-direction)
Jz = np.zeros(nx)  # Current density (z-direction)
My = np.zeros(nx)  # Magnetization (y-direction)
...
for n in range(time_steps):
    # Update Hy field
    for i in range(nx - 1):
        Hy[i] = ch1 * ((Ez[i + 1] - Ez[i]) / dx - My[i]) + ch2 * Hy[i]

    # Update Ez field
    for i in range(1, nx):
        Ez[i] = ce1 * ((Hy[i] - Hy[i - 1]) / dx - Jz[i]) + ce2 * Ez[i]

    # Apply source (sinusoidal current density with Gaussian envelope)
    Jz[:] = np.sin(2 * np.pi * source_frequency * n * dt) * gaussian_envelope
...
ani = animation.FuncAnimation(fig, update, frames=len(Ez_history), interval=50, blit=False)
plt.show()
